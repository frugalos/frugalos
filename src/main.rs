...
fn main() {
    let matches = App::new("frugalos")
        ...
        .subcommand(
            ...
            SubCommand::with_name("delete-objects-by-ids")
    ...
    if let Some(matches) = matches.subcommand_matches("create") {
    ...
    } else if let Some(matches) = matches.subcommand_matches("delete-objects-by-ids") {
        // TAKE SNAPSHOT
        let logger = track_try_unwrap!(logger_builder.build());
        ...
        track_try_unwrap!(frugalos::daemon::delete_objects_by_ids(
            &logger, rpc_addr, bucket, device_id, objects
        ));
