...
tmux send-keys -t $WIN.0 "FRUGALOS_REPAIR_ENABLED=1 FRUGALOS_SNAPSHOT_THRESHOLD=1 bin/frugalos start --data-dir srv0 ${FRUGALOS_START_FLAGS} --http-server-bind-addr 127.0.0.1:${PORT}" C-m
sleep 6
...
do
    ...
    sleep 1
    tmux send-keys -t ${server}.0 "FRUGALOS_REPAIR_ENABLED=1 FRUGALOS_SNAPSHOT_THRESHOLD=1 bin/frugalos start --data-dir ${server} ${FRUGALOS_START_FLAGS} --http-server-bind-addr 127.0.0.1:${http}" C-m
    sleep 3
done
